...
router.get('/policeData',[loging],async(req,res)=>{
    const result= await policeRegisisterModel.findById(req.user._id).select('-password')
    res.send(result)
})

router.post('/policelogins',async (req,res)=>{

    let {error} = validate(req.body)
    if(error) return res.status(400).send(error.details[0].message)

    let valid= await policeRegisisterModel.findOne({mail_id:req.body.mail_id})
